...
clean_excel_output <- function(res) {
  map(res, pluck,"stations") %>% 
    ...
    write_csv(usethis::proj_path("data/0_basic/station_raw.csv"))
  map(res,pluck, "phenology") %>% 
    reduce(bind_rows) %>% 
    mutate(period = case_when(period=="早"~"early",
                              period=="中"~"mid",
                              TRUE~"late")) %>% 
    write_csv(here::here("data/0_basic/phenology_raw.csv"))
}
...
get_phenology_repaired <- function(nearest_ref) {
  read_csv(here::here("data/0_basic/phenology_raw.csv")) %>% 
    left_join(nearest_ref, by = c("station"="raw_station")) %>% 
    mutate(station_id = if_else(is.na(station_id), station,station_id)) %>% 
...
write_csv(
  ...
  here::here("data/0_basic/station.csv")
)
write_csv(
...
# saved at here::here("data/0_basic/climate_raw")
station <- read_csv(here::here("data/0_basic/station.csv"))

decade_path <- function(decade = 1990,type = "TEM") {
  yearList <- dir(here::here("data/0_basic/weather2020/"),
    ...
    str_sub(1,4) %>% 
    as.numeric() 
  dir(here::here("data/0_basic/weather2020/"),
      pattern = paste0("SURF_CLI_CHN_MUL_DAY-",type),
...
read_raw_weather <- function(decade = 1990) {
  if(!file.exists(here::here("data/0_basic/climate_raw"))) {
    dir.create(here::here("data/0_basic/climate_raw"))
  }
...
map_dfr(c(1990,2000,2010,2020), read_raw_weather) %>% 
  write_csv(here::here("data/0_basic/climate_raw.csv"))
...
read_csv(here::here("data/0_basic/climate_raw.csv")) %>% 
  extreme_to_na() %>% 
  do_the_impute()%>% 
  distinct() %>% 
  mutate(year_date = str_c(station,"-",year,"-",date)) %>% 
  write_csv(here::here("data/0_basic/climate_slide_imputed.csv"))
